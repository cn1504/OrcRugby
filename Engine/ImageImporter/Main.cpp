...
std::shared_ptr<Core::Logger> ImageImporter::out;

using namespace ImageImporter;
...
int main(int argc, char* argv[])
{
	try {
		...
		out = std::make_shared<Core::Logger>("ImageImporter.txt");
		...
		else
		{
			...
			ofd->Title = _T("Choose PNG File To Import");
			if (ofd->ShowDialog())
			{
				load(ofd->FileName);
			}
