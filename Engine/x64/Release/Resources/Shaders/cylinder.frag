...
layout(location = 3) out vec4 outSpecularHardness;
layout(location = 4) out vec4 outEmanative;
...
{	
	...
	else
	{
		...
		outNormal = vec4(normalize(qi - ui) * 0.5 + 0.5, 1.0);
	...
	outSpecular = mix(SpecularColor, SpecularColor2, t);
	outSpecularHardness = vec4(vec3(mix(SpecularHardness, SpecularHardness2, t) / 512), 1.0);
	outEmanative = mix(EmanativeColor, EmanativeColor2, t);
}